...
/**
he basic idea is to pick each offer, and subtract the needs. And then compute the price without the offer.
	...
	public int shoppingOffers(List<Integer> price, List<List<Integer>> special, List<Integer> needs) {
	    ...
	    for(int i = 0; i < special.size(); i++) {
	        ...
	        for(int j = 0; j < needs.size(); j++) { // subtract offer items from needs
	            ...
	            needs.set(j, remain);
	            if(validOffer && remain < 0) validOffer = false; // if offer has more items than needs
	        }
	        if(validOffer) { //if valid offer, add offer price and recurse remaining needs
