...
public class BinaryTreeSummary {
	    ...
	    /** 
	     * original url: http://blog.csdn.net/fightforyourdream/article/details/16843303
	    ...
	    //      System.out.println(getNodeNum(r1));  
	          ...
	          System.out.println(getDepthByTrackingLastLevelNode(r1));
	    ...
	    //      inorderTraversal(mirrorRoot);  
	        ...
	        public static int getDepth(TreeNode root) {  
	         ...
	         //another solution to just track the level's lasy node
	        public static int getDepthByTrackingLastLevelNode(TreeNode root) {  
	            if(root == null){  
	                return 0;  
	            }  
	            ...
	            while( !queue.isEmpty() ){  
	                ...
	                if(cur==lastLevelNode){ // 说明已经遍历完当前层的所有节点  
	                    depth++;                       // 增加高度, after previous level is processed
	                    //last level node's children will be next last level node
	                    lastLevelNode = lastLevelNode.right==null?lastLevelNode.left:lastLevelNode.right;
	                }  
	                if(cur.left != null){               // 如果有左孩子，加到队尾  
	                    queue.add(cur.left);
	        ...
	        /** 
	         *  递归转换BST为双向链表(DLL). try to remember this algorithm.
	         */  
	        ...
	        /** 
	         *  求二叉树第K层的节点个数   迭代解法：  
	         *  同getDepth的迭代解法 . just add additional condition to stop at level k
	         */  
	        ...
	        // 1. 破坏原来的树，把原来的树改成其镜像  
	        public static void mirrorItor(TreeNode root) {  
	            if(root == null){  
	                return;  
	            }  
	        ...
	        // 2. 不能破坏原来的树，返回一个新的镜像树  
	        public static TreeNode mirrorCopyItor(TreeNode root){  
	            if(root == null){  
	                return null;  
	            }  
	         ...
	         */  
	        public static TreeNode getLowestCommonParentRec(TreeNode root, TreeNode n1, TreeNode n2) {  
	            if (findNodeRec(root.left, n1)) {               // 如果n1在树的左子树  
	                if (findNodeRec(root.right, n2)) {      // 如果n2在树的右子树  
	                    return root;                                // 返回根节点  
	                } else {            // 如果n2也在树的左子树  
	                    return getLowestCommonParentRec(root.left, n1, n2); // 递归处理  
	                }  
	            } else {                // 如果n1在树的右子树  
	                if (findNodeRec(root.left, n2)) {           // 如果n2在左子树  
	                    return root;  
	                } else {                 // 如果n2在右子树  
	                    return getLowestCommonParentRec(root.right, n1, n2); // 递归处理  
	                }  
	            }  
	        }  
	        ...
	        // 求二叉树中两个节点的最低公共祖先节点 （更加简洁版的递归）   LCA rec
	        ...
	        /** 
	         * 非递归解法：LCA common, applies to r-way tree  
